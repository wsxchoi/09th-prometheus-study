...
<img width="547" height="400" alt="image" src="https://github.com/user-attachments/assets/16296d58-dcc6-4aac-a438-ccb411ff3a0d" />
...
<img width="696" height="433" alt="image" src="https://github.com/user-attachments/assets/e4d8c2c2-3e8d-4501-a2c6-38e5534ac108" />

실제 구현은 **Copy-on-Write (CoW)** 기법을 많이 사용합니다. 스냅샷 생성 시 데이터를 실제로 복사하지 않고, 이후 원본이 변경될 때만 변경 전 데이터를 별도 공간에 보존합니다.

### 학부 데이터베이스 수업에서 쉐도우 페이징도 배웠는데 동작은 비슷하면서 목적은 조금 다르다.
Shadow Paging은 트랜잭션 원자성(Atomicity)을 보장하기 위한 recovery 메커니즘이다.
- 트랜잭션이 실패하면 shadow table로 롤백하는 게 목적
- 트랜잭션이 성공하면 shadow를 current로 교체 (포인터 스왑)
- 매 트랜잭션마다 생성/소멸을 반복
- WAL의 대체재 (로그 없이 복구 가능)
<img width="876" height="724" alt="image" src="https://github.com/user-attachments/assets/c323784a-4278-495f-8f6a-1d0bd02cc3f7" />
