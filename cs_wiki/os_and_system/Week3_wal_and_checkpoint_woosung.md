...
## 2. Write Amplification (쓰기 증폭)
### **개념**

실제로 쓰려는 데이터보다 더 많은 양이 디스크에 기록되는 현상이다. SSD는 데이터를 기록하는 방식 때문에 물리적인 쓰기 한도가 존재하기 때문에, Write Amplification은 SSD의 수명을 단축시킨다.

* WAF

![alt text](./images/waf.png)
Write Amplification의 정도를 WAF(write amplification factor)로 표현한다.

![alt text](./images/ssd.png)

### **발생 원인**

* SSD Page 크기를 고려하지 않은 쓰기 작업

SSD는 데이터를 page(보통 4~16KiB) 단위로만 읽기/쓰기를 할 수 있고, block(수백 KiB) 단위로만 지울 수 있다. 또한 구조적으로 덮어쓰기가 불가능하다. 이 때, 1바이트를 기록하는 개별적인 쓰기 작업을 4번 요청하면, page 크기가 4KiB라 했을때, 실제로는 16KiB를 디스크에 쓰게되면서 Write Amplification이 발생한다.

### **WAL에서의 Write Amplification 최소화**

Prometheus에서는 Page 개념을 도입하여 Write Amplification를 최소화한다.
...
**Write Amplification을 최소화하는 방식**
* 쓰기 정렬

32KiB = SSD 페이지(4~16KiB)의 배수, read-modify-write 방지

* 작은 레코드 batching

작은 레코드의 경우 여러 레코드를 한 page에 모아서 write
