...
## 컨테이너는 Host의 memmory 공유

나름대로 좋은 실습 주제를 로우레벨 시스템설계의 카테고리 안에서 찾고자 고민하던 결과 정신차려보니 week4 주제와 크게 벗어난 실습이 된 점 먼저 사과드립니다...ㅜ
...
[부연 설명]
overcommit_memmory = 1 은 **프로세스가 요청하는 메모리 할당량이 실제로 가능한지 여부를 막론 하고 일단 요청을 받는 것**입니다.  
즉 oom이 발생할 수 밖에 없는 요청이라도 일단 받아서 메모리 할당을 시도하는 것입니다. 실습에서 swapiness = 0 이었기 때문에 cgroup 설정의 350mb를  
초과한 500mb 의 메모리 할당이 허용되지 않아 커널에서 oom을 실행한 것이고 만약 swapiness가 50만 되었어도 해당 요청은 실행되었을 것입니다.  
그렇다면 이러한 옵션이 왜필요하냐? 에 대한 이해는 위의 fork 작업 그리고 웅비님이 부연설명해주신 Redis의 사례로 이해하시면 될 것 같습니다.  

반면 overcommit_memmory = 2는 commitlimit를 반드시 지키는 파라미터 설정입니다. 이는 **swap을 포함하여 계산한 값**이기 때문에,  
swapiness에 전혀 영향을 받지 않습니다. swapiness가 0 이던 100 이던 마지막 추가 실습의 결과는 같았을 것입니다.  
해당 파라미터를 키게 되면 commitlimit 이상의 메모리 할당은 시스템에서 절대 허용하지 않습니다. 이미 예약된 메모리 총합의 장부인  
committed_as는 절대 commitlimit를 넘을 수 없습니다. committed_as 가 commitlimit 를 넘게되는 메모리 할당 요청은 커널에서  
허용조차 하지않고 그 결과가 추가 실습의 결과 입니다. 반대로, 실습 도중 overcommit_ratio가 50인 상태 당시 (commitlimit 가 1.5GB / committed_As:2.3gb)
에서 (이는 당시 oveercommit_memmory 가 1이어서 commitlimit를 무시했기때문에 가능했습니다.)  에서 commitlimit의 확장 없이 바로 oveercommit_memmory를 2로 튜닝한 결과,  
기존의 기본적인 메모리 할당 조차 커널에서 허용하지 않아(시스템 데몬 ssh 등) 서버가 마비되어 버린 것입니다.
